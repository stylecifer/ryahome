<?php
	...
	class UniteFunctionsWPRev{
		...
		public static function import_media($file_url, $folder_name) {
			...
			if(!file_exists($upload_dir.$artDir)) mkdir($upload_dir.$artDir);
			if(!file_exists($upload_dir.$artDir.$folder_name)) mkdir($upload_dir.$artDir.$folder_name);
			...
			if(@fclose(@fopen($file_url, "r"))){ //make sure the file actually exists
				...
				$saveDir = $upload_dir.$artDir.$folder_name.$filename;
				...
				$artdata = array(
					...
					'guid' => $siteurl.'/'.$artDir.$folder_name.$filename,
				...
				$attach_id = wp_insert_attachment($artdata, 'revslider/'.$folder_name.$filename);
